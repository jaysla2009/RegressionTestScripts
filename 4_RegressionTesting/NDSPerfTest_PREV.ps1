	...
	$func = {
        ...
        while ($true){
			try{
                ...
                $t = Get-date
				$currentvalue=[int]($paramvalue.$parameter);
...
Function Load-PerfmonCounters([string]$ConfigPath, [string]$Sensor){
	...
	$LogPath = "{0}\NDSPerfTestReports_{1}_{2}_{3}.csv" -f $CounterPathreport,$TestSuiteName,$Sensor,$Time
	#prev_ver
...
Function RunPerfTest(){
    Try{
		...
		$global:TMPaction = New-Item -ItemType file -Path ($env:TEMP+"\NDSPerfTestReportsTEMP"+$time+".log")
